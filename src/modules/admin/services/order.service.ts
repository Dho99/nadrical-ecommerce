...
function mapBackendOrder(bo: any): OrderWithItems {
  const items: DbOrderItem[] = (bo.order_items || []).map((oi: any) => ({
    ...
    product_name_snapshot: oi.product_name_snapshot || oi.product?.name || 'Product',
    ...
    line_total: Number(oi.line_total || (Number(oi.price || 0) * Number(oi.quantity || 1))),
    created_at: oi.created_at,
    updated_at: oi.updated_at,
  }))

  let status = bo.status || bo.order_status || 'pending_payment'
  if (status === 'WAITING_CONFIRMATION' || status === 'WAITING_ONGKIR') status = 'pending_payment'
...
export const orderService = {
  async listOrders(): Promise<OrderWithItems[]> {
    try {
      const res = await api.get<{ success: boolean; data: any[] }>('/ecommerce/orders', {
        params: { limit: 100 },
      })
  ...
  async listOrdersPage(
    ...
    try {
      const res = await api.get<{ success: boolean; data: any[]; meta?: any }>('/ecommerce/orders', {
        params: { page, limit },
      })
      if (Array.isArray(res.data?.data) && res.data.data.length > 0) {
