...
export const authService = {
  async register(name: string, email: string, password: string): Promise<AuthSession> {
    ...
    const baseUsername = cleanName.toLowerCase().replace(/[^a-z0-9]/g, '') || cleanEmail.split('@')[0].replace(/[^a-z0-9]/g, '')
    const username = baseUsername.length >= 3 ? baseUsername : `${baseUsername}${Math.floor(100 + Math.random() * 900)}`

    try {
      const res = await api.post<StandardApiResponse<BackendAuthData>>('/auth/register', {
        email: cleanEmail,
        username,
        password,
      })
      ...
      if (!data || !data.akun) {
        throw new Error(res.data.message || 'Failed to parse registration response')
      }
  ...
  async login(emailOrUsername: string, password: string): Promise<AuthSession> {
    const identifier = emailOrUsername.trim()
    try {
      const res = await api.post<StandardApiResponse<BackendAuthData>>('/auth/login', {
        identifier,
        password,
      })
      ...
      const data = res.data.data
      if (!data || !data.akun) {
        throw new Error(res.data.message || 'Failed to parse login response')
      }
      ...
      if (identifier.toLowerCase() === SEED_ADMIN.email.toLowerCase() && password === SEED_ADMIN.password) {
  ...
  async updateProfile(
    ...
    try {
      const res = await api.put<StandardApiResponse<BackendAkun>>(`/core/accounts/${userId}`, {
