...
import type { Product, ProductCategory, ProductFilters, ProductSpec, ProductVariant } from '../types/product.type'
...
export const productService = {
  ...
  async getCategories(): Promise<ProductCategory[]> {
    try {
      const res = await api.get<StandardApiResponse<BackendCategory[]>>('/ecommerce/categories')
      if (Array.isArray(res.data.data) && res.data.data.length > 0) {
  ...
  async updateProduct(id: string, patch: Partial<ProductDraft>): Promise<Product | null> {
    try {
      const res = await api.put<StandardApiResponse<BackendProduct>>(`/ecommerce/products/${id}`, {
        ...(patch.name ? { name: patch.name } : {}),
        ...(patch.base_price !== undefined ? { price: patch.base_price } : {}),
        ...(patch.stock !== undefined ? { stock: patch.stock } : {}),
        ...(patch.cover_image_url ? { cover_image_url: patch.cover_image_url } : {}),
